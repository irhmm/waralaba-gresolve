...
BEGIN
  ...
  IF caller_role IS NULL OR caller_role NOT IN ('franchise','super_admin') THEN
  ...
  SELECT
    ...
    (RANK() OVER (ORDER BY (b.admin_inc + b.worker_inc - b.exp_total - b.ps_total) DESC))::int AS rank_position
  FROM base b
  ORDER BY omset DESC;
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_franchise_ranking(text) TO authenticated;
