-- Create function to calculate and update franchise profit sharing data
...
BEGIN
  ...
  SELECT 
    ...
    20.00 as admin_percentage, -- default 20%
    80.00 as franchise_percentage, -- default 80%
  ...
  LEFT JOIN (
    ...
    WHERE ai.tanggal >= (target_month_year || '-01')::DATE
      AND ai.tanggal <= (target_month_year || '-31')::DATE
    GROUP BY ai.franchise_id
  ) revenue_data ON f.id = revenue_data.franchise_id
  ON CONFLICT (franchise_id, month_year) 
  DO UPDATE SET
    total_revenue = EXCLUDED.total_revenue,
    share_nominal = EXCLUDED.total_revenue * franchise_profit_sharing.admin_percentage / 100,
    updated_at = now();
...
-- Create function to get franchise profit sharing data
