-- Fix ambiguous column errors and improve profit sharing calculation functions

-- Drop and recreate the get_franchise_profit_sharing_data function with proper column qualifications
DROP FUNCTION IF EXISTS public.get_franchise_profit_sharing_data(text);
...
BEGIN
  ...
  -- Return the data with explicit table aliases
  RETURN QUERY
  SELECT 
    fps.franchise_id,
...
-- Update calculate_franchise_profit_sharing to use franchise-specific settings properly
DROP FUNCTION IF EXISTS public.calculate_franchise_profit_sharing(text);
...
BEGIN
  ...
  SELECT gps.admin_percentage, gps.franchise_percentage
  ...
  franchise_settings AS (
    SELECT 
      f.id AS fr_id,
      -- Priority: franchise_profit_settings > global_profit_settings > defaults
...
-- Create function to recalculate all months when settings change
CREATE OR REPLACE FUNCTION public.recalculate_profit_sharing_for_settings_change(changed_franchise_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  month_record RECORD;
BEGIN
  -- Get all existing months for this franchise and recalculate them
  FOR month_record IN 
    SELECT DISTINCT month_year 
    FROM public.franchise_profit_sharing 
    WHERE franchise_id = changed_franchise_id
  LOOP
    PERFORM public.calculate_franchise_profit_sharing(month_record.month_year);
  END LOOP;
END;
$function$;